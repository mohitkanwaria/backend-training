...
const getXINRBooks = async function (req, res) {
    // let indianBooks = await bookModel.find ({:{ $in: ["100", "300", "500"] }})

    let indianBooks = await bookModel.find({$or:[{price:{indianPrice:"500"}},{price:{indianPrice:"200"}},{price:{indianPrice:"700"}}]})

     res.send({ msg: indianBooks })
}

const getRandomBooks = async function (req, res) {
    let randomBooks = await bookModel.find({ $and: [{ stockAvailable: true }, { totalpages: { $gte: 500 } }] })//if i put or operator in middle of (,)it just skip the second condn and focus on first condn.
...
//         { authorName : "SK"},//condition
//         {$set : data}, //update in data
//         { new : true , upsert : true}, //new : true - will you give back the updated command //upsert : it finds and updates the document but if the doc is not found (i.e it does not exist ) then it creates a new document i.e UPdate or inSERT
//      )
//     res.send({msg : allBooks})
// }
